        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="fixed inset-0 bg-black text-white flex flex-col items-center justify-center space-y-12 text-5xl z-40 shadow-2xl"
            >
              ...
              {menuItems.map((item) => (
                ...
                >
                  <Link
                    href={item.href}
                    onClick={toggleMenu}
                    className="hover:text-gray-400 transition-colors font-bold"
                  >
                    {item.label}
                  </Link>
                </motion.div>
              ...
              >
                <motion.div
                  whileHover={{ x: [0, -3, 3, -3, 0] }}
                  transition={{ duration: 0.4 }}
                  className="relative overflow-hidden bg-white text-black px-10 py-4 rounded-full font-bold text-2xl border border-white"
                >
                ...
                <motion.div
                  whileHover={{ x: [0, -3, 3, -3, 0] }}
                  transition={{ duration: 0.4 }}
                  className="relative overflow-hidden bg-white text-black p-4 rounded-full flex items-center justify-center border border-white"
                >
                  <span className="absolute inset-0 bg-black translate-x-[-101%] group-hover:translate-x-0 transition-transform duration-500 ease-in-out" />
                  <span className="relative z-10 transition-colors duration-500 group-hover:text-white">
